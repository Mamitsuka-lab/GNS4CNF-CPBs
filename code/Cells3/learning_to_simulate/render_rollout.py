...
flags.DEFINE_string("data_pref", None, help="Path to output file")
...
LOG_PATH = "%s/log_erros_rollout3.txt" % config.DATA_DIR
LOG_PATH_A = "%s/log_erros_rollout3_all_steps.txt" % config.DATA_DIR
...
def main4(unused_argv):
    ...
    video_path = "%s/%s_%s.avi" % (config.DATA_DIR, FLAGS.data_pref, FLAGS.test_id)
    ...
    for i, im in enumerate(img_seq[OFFSET + FLAGS.rollout_offset:]):
        ...
        fout_a.write(
            "%s,%s,%s,%.5f,%.5f,%.5f" % ("%s_%s" % (FLAGS.data_pref, FLAGS.test_id), FLAGS.rollout_offset, check_offset(iFrame + 1), d1, d2, d3))
        ...
        fig, ax = plt.subplots()
        if iFrame >= config.C + FLAGS.rollout_offset:
            pref = "Prediction: "
            color = "blue"
    ...
    save_gifanim(img_frames, "%s_%s_%s_gb.gif" % (FLAGS.out_path, FLAGS.test_id, FLAGS.rollout_offset))
    fout_a.close()
    ...
    def update(step_i):
        ...
        for ia, ax in enumerate(axes):
            ...
            fig.suptitle('%s\nCor @%3s: %s' % (pref,step_i, cor_lists[ii]), color=color)
    ...
    # plt.show(block=FLAGS.block_on_show)
    writergif = animation.PillowWriter(fps=2)
    unused_animation.save("%s_%s_%s_sides.gif" % (FLAGS.out_path, FLAGS.test_id, FLAGS.rollout_offset), writergif,
