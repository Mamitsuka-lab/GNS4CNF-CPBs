...
flags.DEFINE_integer('test_id', 0, help='Test Id.')
flags.DEFINE_integer('rollout_offset', 0, help='Rollout offset.')

flags.DEFINE_integer('num_steps', 300000, help='Number of steps of training.')
...
flags.DEFINE_string('data_pref', "hmsc",
                    help='The path for saving outputs (e.g. rollouts).')
...
import config
INPUT_SEQUENCE_LENGTH = config.C  # So we can calculate the last C-1 velocities.
...
def prepare_rollout_inputs(context, features):
    ...
    out_dict = {**context}
    print("Preprare rollout f: ", features['position'].shape)
    # Position is encoded as [sequence_length, num_particles, dim] but the model
    # expects [num_particles, sequence_length, dim].
    ...
    print("Output of preparing rollout postion: ", out_dict['position'].shape)
...
def get_input_fn2(data_path, batch_size, mode, split):
  ...
  """
    ...
    def input_fn():
        ...
        metadata = _read_metadata2(data_path, FLAGS.data_pref)
        """Input function for learning simulation."""
        # Loads the metadata of the dataset.
        # Create a tf.data.Dataset from the TFRecord.
        dss = []
        data_paths = []
        print("MOde, split: ", mode, split)
        if data_path.__contains__("Cells"):
            if split == "train":
                for path in glob.glob("%s/train_%s_*.tfrecord" % (data_path,  FLAGS.data_pref)):
                    # if not path.split("/")[-1].__contains__("%s"% FLAGS.test_id):
                    data_paths.append(path)
            elif split.__contains__("test"):
                print("Loading test data...", data_path, FLAGS.test_id)
                data_paths.append("%s/test_%s_%s.tfrecord" % (data_path,  FLAGS.data_pref, FLAGS.test_id))
            else:
                print("Error: No %s" % split)
                exit(-1)
        else:
            data_paths.append(os.path.join(data_path, f'{split}.tfrecord'))
        for data_pathx in data_paths:
            ds = tf.data.TFRecordDataset([data_pathx])
            ds = ds.map(functools.partial(
                reading_utils.parse_serialized_simulation_example, metadata=metadata))
            if mode.startswith('one_step'):
                # Splits an entire trajectory into chunks of C steps.
                # Previous C-2 velocities, current velocity and target.
...
def rollout2(simulator, features, num_steps):
    """Rolls out a trajectory by applying the model in sequence."""
    initial_positions = features['position'][:, 0 + FLAGS.rollout_offset:INPUT_SEQUENCE_LENGTH + FLAGS.rollout_offset]
    ground_truth_positions = features['position'][:, INPUT_SEQUENCE_LENGTH + FLAGS.rollout_offset:]
    global_context = features.get('step_context')
    print("In rollout feature position shape: ", features['position'].shape)
    def step_fn(step, current_positions, predictions):
...
def get_rollout_estimator_fn2(data_path,
                              noise_std,
                              latent_size=128,
                              hidden_size=128,
                              hidden_layers=2,
    ...
    def estimator_fn(features, labels, mode):
        ...
        num_steps = metadata['r_steps'] - INPUT_SEQUENCE_LENGTH - FLAGS.rollout_offset
        print("N_STEPS: ", num_steps)
        print("F SHAPE: ", features['position'].shape)
        rollout_op = rollout2(simulator, features, num_steps=num_steps)
...
def _read_metadata2(data_path,data_pref):
    with open(os.path.join(data_path, 'metadata_%s.json' %data_pref), 'rt') as fp:
...
#             get_rollout_estimator_fn2(FLAGS.data_path, FLAGS.noise_std),
#             model_dir=FLAGS.model_path)
#
...
#             input_fn=get_input_fn2(FLAGS.data_path, batch_size=1,
...
def main2(_):
    """Train or evaluates the model."""
    print("Data path, test id, data_pref: ", FLAGS.data_path, FLAGS.test_id, FLAGS.data_pref, FLAGS.rollout_offset)
    config.DATA_PREX  = FLAGS.data_pref
    if FLAGS.mode in ['train', 'eval']:
        estimator = tf.estimator.Estimator(
            get_one_step_estimator_fn(FLAGS.data_path, FLAGS.noise_std),
            model_dir= FLAGS.model_path)
        if FLAGS.mode == 'train':
            # Train all the way through.
            estimator.train(
                input_fn=get_input_fn2(FLAGS.data_path, FLAGS.batch_size,
                                       mode='one_step_train', split='train'),
                max_steps=FLAGS.num_steps)
        else:
            # One-step evaluation from checkpoint.
            eval_metrics = estimator.evaluate(input_fn=get_input_fn2(
                FLAGS.data_path, FLAGS.batch_size,
                mode='one_step', split=FLAGS.eval_split))
            logging.info('Evaluation metrics:')
            logging.info(eval_metrics)
        ...
        # Iterate through rollouts saving them one by one.
        metadata = _read_metadata2(FLAGS.data_path, FLAGS.data_pref)
        rollout_iterator = rollout_estimator.predict(
            input_fn=get_input_fn2(FLAGS.data_path, batch_size=1,
                                   mode='rollout', split=FLAGS.eval_split))
        ...
        for example_index, example_rollout in enumerate(rollout_iterator):
            example_rollout['metadata'] = metadata
            filename = f'rollout_{FLAGS.eval_split}_{example_index}_{FLAGS.rollout_offset}.pkl'
            filename = os.path.join(FLAGS.output_path, filename)
            logging.info('Saving: %s.', filename)
            if not os.path.exists(FLAGS.output_path):
                os.mkdir(FLAGS.output_path)
