...
def create_bash_script(bash_dir):
    ...
    f_script.write("    echo \"Need 1 argument: data_pref \"\n")
    f_script.write("    exit -1\n")
    f_script.write("fi\n")
    f_script.write("source ~/anaconda3/etc/profile.d/conda.sh\n")
    f_script.write("conda activate Cells\n")
    f_script.write("python -m learning_to_simulate.train --data_pref=$1 --data_path=%s --model_path=%s/$1_%s\n"
    ...
    f_script.write("    echo \"Need 3 argument: data_pref test_id rollout_offset\"\n")
    f_script.write("    exit -1\n")
    f_script.write("fi\n")
    f_script.write("source ~/anaconda3/etc/profile.d/conda.sh\n")
    f_script.write("conda activate Cells\n")

    f_script.write("python -m learning_to_simulate.train --data_pref=$1 --test_id=$2 --rollout_offset=$3 --mode=\"eval_rollout\" --data_path=%s "
                   "--model_path=%s/$1_%s "
    ...
    f_script.write("#!/bin/bash\n")
    f_script.write("if [ $# -ne 3 ];then\n")
    f_script.write("    echo \"Need 2 argument: data_pref test_id rollout_offset\"\n")
    f_script.write("    exit -1\n")
    f_script.write("fi\n")
    f_script.write("source ~/anaconda3/etc/profile.d/conda.sh\n")
    f_script.write("conda activate Cells\n")
    f_script.write("python -m learning_to_simulate.render_rollout --data_pref=$1 "
                   "--rollout_path=%s/\"$1\"_%s_\"$2\"/rollout_test_0_$3.pkl "
                   "--out_path=%s/\"$1\"_%s_$2 "
                   "--test_id=$2 --rollout_offset=$3\n" % (config.ROLLOUT_DIRS,params, config.DATA_DIR,params))
