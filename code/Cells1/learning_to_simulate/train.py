...
flags.DEFINE_integer('test_id', 0, help='Rollout test id.')
...
def get_input_fn2(data_path, batch_size, mode, split):
  ...
  """
    ...
    def input_fn():
        ...
        metadata = _read_metadata2(data_path,split)
        """Input function for learning simulation."""
        # Loads the metadata of the dataset.
        ...
        if data_path.__contains__("Cells"):
            if split == "train":
                ...
                data_paths.append("%s/test_%s.tfrecord" % (data_path, config.TEST_ID))
            else:
...
def get_one_step_estimator_fn(data_path,
    ...
    metadata = _read_metadata2(data_path, "train")
...
def get_rollout_estimator_fn2(data_path,
    ...
    def estimator_fn(features, labels, mode):
        ...
        num_steps = metadata['r_steps'] - INPUT_SEQUENCE_LENGTH
...
def _read_metadata2(data_path, split):
    suffix = split
    if split.__contains__("test"):
        suffix = "test"
    with open(os.path.join(data_path, 'metadata_%s.json' % suffix), 'rt') as fp:
        return json.loads(fp.read())
#
# def main(_):
...
def main2(_):
    ...
    if FLAGS.mode in ['train', 'eval']:
        ...
        if not FLAGS.output_path:
            raise ValueError('A rollout path must be provided.')
        print("Load rolling model...")
        rollout_estimator = tf.estimator.Estimator(
            get_rollout_estimator_fn2(FLAGS.data_path, FLAGS.noise_std),
            model_dir=FLAGS.model_path)
        print("Start rolling...")
        # Iterate through rollouts saving them one by one.
        metadata = _read_metadata2(FLAGS.data_path,"test")
        rollout_iterator = rollout_estimator.predict(
            input_fn=get_input_fn2(FLAGS.data_path, batch_size=1,
                                   mode='rollout', split=FLAGS.eval_split))
        ...
        for example_index, example_rollout in enumerate(rollout_iterator):
            example_rollout['metadata'] = metadata
            filename = f'rollout_{FLAGS.eval_split}_{example_index}_{config.TEST_ID}.pkl'
            filename = os.path.join(FLAGS.output_path, filename)
            logging.info('Saving: %s.', filename)
            if not os.path.exists(FLAGS.output_path):
                os.mkdir(FLAGS.output_path)
