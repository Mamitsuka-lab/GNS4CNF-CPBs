...
LOG_PATH = "%s/log_erros_rolloutx.txt" % config.DATA_DIR
LOG_PATH_A = "%s/log_erros_rolloutx_all_steps.txt" % config.DATA_DIR
...
def main3(unused_argv):
    ...
    def update(step_i):
        ...
        for j, (ax, trajectory) in enumerate(plot_info):
            ...
            ax.set_xlim(bounds[0][0], bounds[0][1])
            ax.set_ylim(bounds[1][0], bounds[1][1])
            ax.set_title("%s%s" % (labels[j], check_offset(step_ii + 1)), color=color)
        ...
        if step_i <= min_x - 1:
            ...
            fig.suptitle('%s: Error @%3s: %s' % (pref, check_offset(step_ii + 1), dis_text), color=color)
            if step_i == min_x - 1:
                _, d1s, d2s, d3s = dist.get_all_dist(trajectories[0][step_i], trajectories[0][config.C - 1], both=True)

                parts = FLAGS.out_path.split("/")
                data_name, conf = parts[-2], parts[-1]
                fout = open(LOG_PATH, "a")
                fout.write("%s,%s,%s,%.5f,%.5f,%.5f\n" % (data_name, conf, FLAGS.test_id, d1, d2, d3))
                fout.write("%s,%s,%s,%.5f,%.5f,%.5f\n" % (data_name, "%s#Static" % conf, FLAGS.test_id, d1s, d2s, d3s))
    ...
    unused_animation.save("%s_%s_graph_dark.gif" % (FLAGS.out_path, FLAGS.test_id), writergif,
...
def expand_img(img, dwidth, color="black"):
    return ImageOps.expand(img, dwidth, fill=color)
...
def main5(unused_argv):
    ...
    cor_lists = []
    OFFSET = 14
    for i, im in enumerate(img_seq[OFFSET:]):
        i += OFFSET
        pref = "Initialization"
        color = "green"
        ...
        dis_text, d1, d2, d3, dsx, dsx2 = dist.get_all_dist4(pos0, posp, both=True, n=config.N_PN, ks = ks)
        if i < 20:
            dis_text = "1.00000"
        cor_lists.append(dis_text)
        fout_a.write("%s,%s,%s,%.5f,%.5f,%.5f" % (data_name, FLAGS.test_id, check_offset(i+1), d1, d2, d3))
        ...
        fig.suptitle('%s  \n Cor @Step %3s: %s' % (pref, check_offset(i + 1), dis_text), color=color, fontsize=8)
        ...
        if i == len(img_seq) - 1:
            ...
            _, d1s, d2s, d3s, dsxs, dsxs2 = dist.get_all_dist4(pos0, start_pos, both=True, n=config.N_PN, ks = ks)


            _, d1n, d2n, d3n, dsxn, dsxn2 = dist.get_all_dist4(pos0, ran_dom_p, both=True, n=config.N_PN, ks = ks)


            fout = open(LOG_PATH, "a")
            fout.write("%s,%s,%s,%.5f,%.5f,%.5f" % (data_name, "%s#GNet" % conf, FLAGS.test_id, d1,d2, d3))
            for jj in range(config.N_PN):
            ...
            fout.write("\n")
            fout.write("%s,%s,%s,%.5f,%.5f,%.5f" % (
                data_name, "%s#Random" % conf, FLAGS.test_id, d1n,d2n, d3n))
            for jj in range(config.N_PN):
    ...
    save_gifanim(img_frames, "%s_%s_bg.gif" % (FLAGS.out_path, FLAGS.test_id))
    fout_a.close()
    ...
    def __revert_id(step_i):
        return step_i - config.C - FLAGS.rollout_offset
    ...
    def update(step_i):
        ...
        for ia, ax in enumerate(axes):
            ...
            corr = cor_lists[ci]
            fig.suptitle('%s\nCor @%3s: %s' % (pref, step_i, corr ), color=color)

    unused_animation = animation.FuncAnimation(
        fig, update,
        frames=np.arange(OFFSET + 1 , len(img_seq)  + 1, FLAGS.step_stride), interval=1000)
    # frames=np.arange(config.C - 1,config.C + 5, FLAGS.step_stride), interval=1000)

    # plt.show(block=FLAGS.block_on_show)
    writergif = animation.PillowWriter(fps=2)
    unused_animation.save("%s_%s_sides.gif" % (FLAGS.out_path, FLAGS.test_id), writergif,
                          savefig_kwargs={'facecolor': 'auto'})
