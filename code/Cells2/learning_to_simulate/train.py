...
flags.DEFINE_integer('num_steps', 150000, help='Number of steps of training.')
  ...
  """
    ...
    def input_fn():
        ...
        if data_path.__contains__("Cells"):
            if split == "train":
                for path in glob.glob("%s/train*.tfrecord" % data_path):
                ...
                data_paths.append("%s/test_%s.tfrecord" % (data_path, FLAGS.test_id))
            else:
...
def main2(_):
    """Train or evaluates the model."""
    print("Data path, test id: ", FLAGS.data_path, FLAGS.test_id, FLAGS.rollout_offset)
    if FLAGS.mode in ['train', 'eval']:
        estimator = tf.estimator.Estimator(
            get_one_step_estimator_fn(FLAGS.data_path, FLAGS.noise_std),
            model_dir= FLAGS.model_path)
