...
def convert_2_sequence_example(slice_2_point_array, serilized=False):
    assert type(slice_2_point_array) == np.ndarray
    example = make_sequential_example(poss=slice_2_point_array, serialized=serilized)
    return example
...
def convert_files(dir_path, test_on_train=False):
    ...
    for i in range(n_files):
        ...
        example = convert_2_sequence_example(slice_2_point_array, serilized=True)

        write_tfrecord(example, "%s/train_%s.tfrecord" % (dir_path, i))
        write_tfrecord(example, "%s/test_%s.tfrecord" % (dir_path, i))
    ...
    print(metainfo)
    with open("%s/metadata.json" % dir_path, "w") as f:
...
if __name__ == "__main__":
    ...
    for dir_path in glob.glob("%s/*_*/"%config.DATA_DIR):
        convert_files(dir_path=dir_path, test_on_train=False)
